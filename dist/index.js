(function (global, factory) {
  ...
  class SimpleSchemeParser {
    ...
    parseDefine(elements, location) {
      ...
      console.log(
        "DEBUG: parseDefine - Extended.List check:",
        name instanceof Extended.List
      );
      // Handle function definition: (define (func-name args) body)
      if (
        (name instanceof Extended.List && name.elements.length > 0) ||
        ...
        console.log(
          "DEBUG: parseDefine - funcName type:",
    ...
    parseLambda(elements, location) {
      ...
      console.log(
        "DEBUG: parseLambda - paramsExpr type:",
        paramsExpr.constructor.name
      );
      ...
      if (paramsExpr instanceof Extended.List) {
        ...
        params = paramsExpr.elements.filter(
          e => e instanceof Atomic.Identifier
        );
      } else if (
        paramsExpr instanceof Atomic.Application &&
        ...
        params.push(
          ...paramsExpr.operands.filter(e => e instanceof Atomic.Identifier)
        );
    ...
    parseLet(elements, location) {
      ...
      if (bindingsExpr instanceof Extended.List) {
        for (const binding of bindingsExpr.elements) {
          if (
            binding instanceof Extended.List &&
    ...
    parseCond(elements, location) {
      ...
      for (const clause of clauses) {
        if (clause instanceof Extended.List && clause.elements.length >= 2) {
          ...
          if (
            predicate instanceof Atomic.Identifier &&
            predicate.name === "else"
          ) {
            catchall = consequent;
          } else {
  ...
  function evaluateExpression(expr, context, control, stash) {
    if (expr instanceof Atomic.NumericLiteral) {
      ...
      control.push(
        createCondInstr(expr.predicates, expr.consequents, expr.catchall)
      );
  ...
  function evaluateInstruction(instruction, context, control, stash) {
    switch (instruction.instrType) {
      ...
      default:
        throw new Error(
          `Unsupported instruction type: ${instruction.instrType}`
        );
    }
  }
    ...
    __verifyAlive() {
      if (!this.__isAlive)
        throw new ConductorInternalError(
          `Channel ${this.name} has been closed`
        );
    }
    /**
  ...
  class Conduit {
    ...
    unregisterPlugin(plugin) {
      ...
      for (
        let i = this.__plugins.length - 1, e = this.__plugins.length - p;
        i >= e;
        --i
      ) {
        ...
        this.__serviceHandlers = new Map([
          [
            ...
            function helloServiceHandler(message) {
              if (
                message.data.version < 0 /* Constant.PROTOCOL_MIN_VERSION */
              ) {
                ...
                console.log(
                  `Host is using protocol version ${message.data.version}`
                );
              }
            },
          ],
  ...
  function encode$1(identifier) {
    if (
      JS_KEYWORDS$1.includes(identifier) ||
      identifier.startsWith("$scheme_")
    ) {
      return (
        "$scheme_" +
        b64Encode$1(identifier).replace(
          /([^a-zA-Z0-9_])/g,
