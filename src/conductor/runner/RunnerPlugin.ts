...
export class RunnerPlugin implements IRunnerPlugin {
  ...
  registerModule<T extends IModulePlugin>(
    ...
    return this.registerPlugin(
      moduleClass,
      this.__evaluator as IInterfacableEvaluator
    );
  }
  ...
  async importAndRegisterExternalModule(
    location: string
  ): Promise<IModulePlugin> {
