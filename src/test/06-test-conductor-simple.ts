...
async function testSimpleConductor() {
  ...
  const testCases = [
    ...
    { code: '(+ 3+4i 1+2i)', expected: '4+6i', description: 'Complex addition' },
  ...
  for (const testCase of testCases) {
    try {
      ...
      if (mockConductor.errors.length > 0) {
        console.log(`❌ ${testCase.description}: ${testCase.code} -> Error: ${mockConductor.errors[0].message}`);
        failed++;
      } else if (mockConductor.outputs.length > 0) {
        const actualOutput = mockConductor.outputs[0];
        if (actualOutput === testCase.expected) {
          console.log(`✅ ${testCase.description}: ${testCase.code} -> ${actualOutput}`);
          passed++;
        } else {
          console.log(`❌ ${testCase.description}: ${testCase.code} -> ${actualOutput} (expected ${testCase.expected})`);
          failed++;
        }
      } else {
        console.log(`❌ ${testCase.description}: ${testCase.code} -> No output`);
        failed++;
      }
      
    } catch (error: any) {
      console.log(`❌ ${testCase.description}: ${testCase.code} -> Exception: ${error.message}`);
      failed++;
    }
  }
  ...
  for (const testCase of errorCases) {
    try {
      ...
      if (mockConductor.errors.length > 0) {
        console.log(`✅ ${testCase.description}: ${testCase.code} -> Error correctly caught`);
      } else {
        console.log(`❌ ${testCase.description}: ${testCase.code} -> Expected error but got: ${mockConductor.outputs[0] || 'no output'}`);
      }
    } catch (error: any) {
      console.log(`✅ ${testCase.description}: ${testCase.code} -> Exception correctly caught: ${error.message}`);
    }
  }
}
