...
function testParser() {
  ...
  const testCases = [
    ...
    { code: '"hello"', expected: 'StringLiteral', description: 'String literal' },
    ...
    { code: '(+ 1 2)', expected: 'Application', description: 'Function application' },
    { code: '(define x 5)', expected: 'Definition', description: 'Variable definition' },
    { code: '(if #t 1 2)', expected: 'Conditional', description: 'Conditional expression' },
    { code: '(lambda (x) (+ x 1))', expected: 'Lambda', description: 'Lambda expression' }
  ];
  ...
  for (const testCase of testCases) {
    try {
      ...
      if (actualType === testCase.expected) {
        console.log(`✅ ${testCase.description}: ${testCase.code} -> ${actualType}`);
        passed++;
      } else {
        console.log(`❌ ${testCase.description}: ${testCase.code} -> ${actualType} (expected ${testCase.expected})`);
        failed++;
      }
    } catch (error: any) {
