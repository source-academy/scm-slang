  ...
  switch (nodeType) {
    ...
    case 'Application': {
      ...
      function substitute(nodeAny: any, env: Record<string, any>): any {
        ...
        switch (nodeAny.type) {
          ...
          case 'FunctionApplication':
            return { ...nodeAny, operator: substitute(nodeAny.operator, env), operands: nodeAny.operands.map((o: any) => substitute(o, env)) };
          case 'LambdaExpression': {
            // Avoid capturing: do not substitute parameters that are newly bound here
            const shadowed = Object.keys(env).reduce((acc: Record<string, any>, key) => {
          ...
          case 'Program':
            return { ...nodeAny, body: nodeAny.body.map((b: any) => substitute(b, env)) };
          default:
            return nodeAny;
        }
      }
      return {
        ...
        operator,
        operands,
        toString: function() { return `(${this.operator.toString()} ${this.operands.map((n: any) => n.toString()).join(' ')})`; },
        isContractible: function() {
          const isPrim = this.operator.type === 'Identifier' && ['+', '-', '*', '/'].includes(this.operator.name);
          const allLit = this.operands.every((op: any) => op.type === 'Literal' || (op.isContractible && op.isContractible()));
          const allContractible = this.operands.every((op: any) => op.isContractible ? op.isContractible() : true);
          return (this.operator.type === 'LambdaExpression' && allContractible) || (isPrim && this.operands.every((op: any) => op.type === 'Literal'));
        },
        isOneStepPossible: function () { return this.operands.some((op: any) => op.isOneStepPossible()) || this.isContractible(); },
        contract: function () { return this; },
        oneStep: function () {
          ...
          const isPrim = this.operator.type === 'Identifier' && ['+', '-', '*', '/'].includes(this.operator.name);
          if (isPrim && this.operands.every((op: any) => op.type === 'Literal')) {
          ...
          if (this.operator.type === 'LambdaExpression' && this.operands.every((op: any) => op.isContractible ? op.isContractible() : true)) {
    ...
    case 'Lambda':
      ...
      return { 
        ...
        params: node.params || [], 
        body,
        toString: () => `(lambda (${(node.params || []).map((p: any) => p.name).join(' ')}) ${body.toString()})`,
        isContractible: () => true,
        isOneStepPossible: () => false,
        contract: function() { return this; },
        oneStep: function() { return this; }
    ...
    case 'Sequence':
      ...
      return { 
        type: 'Program', 
        body: bodyNodes,
        toString: function() { return this.body.map((n: any) => n.toString()).join('\n'); },
        isContractible: function() { return this.body.every((expr: any) => expr.isContractible()); },
        isOneStepPossible: function() { return this.body.some((expr: any) => expr.isOneStepPossible()); },
        contract: function() {
...
export function stepExpression(code: string, stepLimit: number = 1000): IStepperPropContents[] {
  try {
    ...
    type PathSegment = { key: 'left' | 'right' | 'body' | 'operands'; index?: number };
    const isOneStepPossibleSafe = (n: any) => !!n && typeof n.isOneStepPossible === 'function' && n.isOneStepPossible();
    const isContractibleSafe = (n: any) => !!n && typeof n.isContractible === 'function' && n.isContractible();
    ...
    const findReduciblePath = (node: any): PathSegment[] | null => {
      ...
      switch (node.type) {
        ...
        case 'BinaryExpression': {
          ...
          if (isOneStepPossibleSafe(left)) return [{ key: 'left' }, ...(findReduciblePath(left) || [])];
          if (isOneStepPossibleSafe(right)) return [{ key: 'right' }, ...(findReduciblePath(right) || [])];
          return isContractibleSafe(node) ? [] : null;
        }
        case 'FunctionApplication': {
          ...
          for (let i = 0; i < ops.length; i++) {
            const sub = ops[i];
            if (isOneStepPossibleSafe(sub)) return [{ key: 'operands', index: i }, ...(findReduciblePath(sub) || [])];
          }
          return isContractibleSafe(node) ? [] : null;
        }
        default:
    ...
    const explainRedex = (node: any): string => {
      ...
      if (node && node.type === 'FunctionApplication' && node.operator && node.operator.type === 'Identifier' && ['+','-','*','/'].includes(node.operator.name)) {
        const ops = node.operands || []
        if (ops.length >= 2) {
          const l = ops[0].raw !== undefined ? ops[0].raw : ops[0].value !== undefined ? String(ops[0].value) : ''
          const r = ops[1].raw !== undefined ? ops[1].raw : ops[1].value !== undefined ? String(ops[1].value) : ''
          if (l !== '' && r !== '') return `Binary expression ${l} ${node.operator.name} ${r} evaluated`
        }
      }
      return 'Step'
    }
    const canReduce = (node: any): boolean => {
      ...
      switch (node.type) {
        ...
        case 'FunctionApplication': {
          ...
          const allLit = ops.length >= 2 && ops.every((o: any) => o.type === 'Literal')
          const allContractible = ops.every((o: any) => (o.isContractible ? o.isContractible() : true))
    ...
    while (currentStep < stepLimit && canReduce(currentNode)) {
      ...
      try {
        ...
        steps.push({
          ast: oldNode,
          markers: [{ redex: beforeRedex, redexType: 'beforeMarker', explanation }]
        });
        
        const afterRedex = getNodeAtPath(currentNode, path) ?? currentNode;
        steps.push({
          ast: currentNode,
          markers: [{ redex: afterRedex, redexType: 'afterMarker', explanation }]
        });
    ...
    if (steps.length === 1 && typeof (currentNode as any).oneStep === 'function') {
      try {
        const oldNode = currentNode;
